...
class Module extends \Fastest\Core\Init
{
    use \Tools, \Singleton;
    ...
    public function countItem()
    {
        ...
        if ($this->caching && !($count = $this->cache->getCompiled($cache))) {
            $count = Q("SELECT COUNT(`id`) AS `count`
                        FROM `#_mdd_" . $this->module['func_name'] . "`
                        WHERE `visible`=1
                        LIMIT 1")->row('count');

            $this->cache->setCache($cache, $count);
        }
    ...
    public function listAction($fields = [], $where = [], $group = [], $order = [], $limit = [ 0, 10 ])
    {
        ...
        if ($this->caching == 1 && !($result = $this->cache->getCompiled($hash))) {
            if (!empty($fields)) {
                $_sql = '';
                $_inc = '';
            ...
            $this->cache->setCache($hash, $result);
        }
    ...
    {
        if (!$this->cache_enable || $this->caching === 0 || ($this->caching === 1 && !($data = $this->cache->getCache($cache)))) {
            return false;
        }
