                ...
                if (!empty($module) && !function_exists($module['func_name']))
                {
                    ...
                    if (class_exists($controller))
                    {
                        ...
                        if (method_exists($controller, $method))
                        {
                            ...
                            $path = PATH_MODULE.DS.$module['func_name'].DS.'tpl'.DS;
                            $template = (isset($data['template']) ? $data['template'] : 'list').'.twig';

                            if (file_exists($path.$template))
                            {
                                $loader = $environment->getLoader();
                                $loader->addPath($path, $module['func_name']);

                                $result = $environment->render('@'.$module['func_name'].'/'.$template, $data);
                            }
                        }
                    }
                }
