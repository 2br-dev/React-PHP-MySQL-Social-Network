...
if (count($user_news_ids) > 0) {
  foreach ($user_news_ids as $id) {
    $comment = Q("SELECT C.id, C.created_at, C.author_id, C.text, C.news_id, N.title, U.avatar, U.name, U.surname, U.sex FROM db_mdd_comments C 
...
$tasks = Q("SELECT T.`id`, T.`created_at`, `from`, `until_date`, U.name, U.surname, U.avatar, U.sex, `until_time`, `text`, `importance` FROM `#_mdd_tasks` T LEFT JOIN db_mdd_users U ON T.`from` = U.id WHERE `for` = ?s ORDER BY T.`id` DESC LIMIT 20", array('for'.$userid))->all();
