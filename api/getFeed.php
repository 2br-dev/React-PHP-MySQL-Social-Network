...
$user_news_ids = Q("SELECT `id` FROM `#_mdd_news` WHERE `author_id` = ?s ORDER BY `id` DESC LIMIT 20", array($userid))->all();
if (count($user_news_ids) > 0) {
  foreach ($user_news_ids as $id) {
    $comment = Q("SELECT C.id, C.created_at, C.author_id, C.text, C.news_id, N.title, U.avatar, U.name, U.surname FROM db_mdd_comments C 
      ...
      WHERE C.news_id = ?s ORDER BY id DESC LIMIT 20", array($id['id']))->all();
    $like = Q("SELECT L.id, L.created_at, L.user, L.news_id, N.title, U.avatar, U.name, U.surname FROM db_mdd_likes L
      ...
      WHERE L.news_id = ?s ORDER BY id DESC LIMIT 20", array($id['id']))->all();
...
$results = Q("SELECT R.id, R.created_at, R.questions, R.user_id, R.estimated_time, R.result, U.avatar FROM db_mdd_tests R 
  ...
  WHERE user_id = ?s AND completed = ?s ORDER BY `id` DESC LIMIT 20", array($userid, 1))->all();
...
$news = Q("SELECT N.id, N.created_at, N.author_id, N.title, N.text, N.importance, U.name, U.surname, U.avatar FROM db_mdd_news N
  LEFT JOIN db_mdd_users U ON N.author_id = U.id ORDER BY `id` DESC LIMIT 20")->all();

$events = Q("SELECT C.id, C.created_at, creator, signed, U.name, U.surname, U.avatar, startDate, endDate, max, title FROM db_mdd_calendar C LEFT JOIN db_mdd_users U ON C.creator = U.id")->all();
